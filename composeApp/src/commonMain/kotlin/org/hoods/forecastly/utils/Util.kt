...
object Util {
    ...
    /*fun formatDateToDay(dateString: String, timeZone: String = TimeZone.currentSystemDefault().id): String {
        val parsedDateTime = LocalDateTime.parse(dateString)
        val convertTimeZone = TimeZone.of(timeZone)
        // Convert to Instant and then ZonedDateTime
        val zonedDateTime = parsedDateTime.toInstant(convertTimeZone).toLocalDateTime(convertTimeZone)
        // Get the day of the week
        return zonedDateTime.dayOfWeek.name.lowercase().replaceFirstChar { it.uppercase() }
    }*/
    fun formatDateToDay(dateString: String, timeZone: String = TimeZone.currentSystemDefault().id): String {
        val parsedDate = LocalDate.parse(dateString)

        // Get the day of the week
        return parsedDate.dayOfWeek.name.lowercase().replaceFirstChar { it.uppercase() }
    }


    fun formatDateToHour(dateString: String, timeZone: String = TimeZone.currentSystemDefault().id): String {
        val parsedDateTime = LocalDateTime.parse(dateString)
        val convertTimeZone = TimeZone.of(timeZone)
        // Convert to Instant and then ZonedDateTime
        val zonedDateTime = parsedDateTime.toInstant(convertTimeZone).toLocalDateTime(convertTimeZone)
        ...
        // Extract hour and minute, and format them
        val hour = zonedDateTime.hour.toString().padStart(2, '0') // Ensure two digits
        val minute = zonedDateTime.minute.toString().padStart(2, '0') // Ensure two digits
        return "$hour:$minute" // Combine into HH:mm format
    }


    fun formatDateToCustom(dateString: String, timeZone: String = TimeZone.currentSystemDefault().id): String {
        val parsedDateTime = LocalDateTime.parse(dateString)
        val convertTimeZone = TimeZone.of(timeZone)
        // Convert to Instant and then ZonedDateTime
        val zonedDateTime = parsedDateTime.toInstant(convertTimeZone).toLocalDateTime(convertTimeZone)
        ...
        // Example custom format: "MMM, d" (e.g., "Nov, 29")
        val month = zonedDateTime.month.name.lowercase().replaceFirstChar { it.uppercase() } // "November"
    ...
    fun isTodayDateWithIsoFormat(day: String): Boolean {
//        val instant = Instant.parse(day) // Parse the ISO-8601 date string
//        instant.toEpochMilliseconds()
//        val inputDay = formatUnixToDay(instant.toEpochMilliseconds())
        val todayDate = formatUnixToDay(Clock.System.now().toEpochMilliseconds())
        return todayDate.lowercase() == day.lowercase()
    }
